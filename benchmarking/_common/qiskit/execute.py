# Notice: This file has been modified for the AIM-QC code base. Original: https://github.com/SRI-International/QC-App-Oriented-Benchmarks/blob/master/_common/qiskit/execute.py
#
...
def set_noise_model(noise_model=None):
    ...
    noise_params = get_noise_parameters(noise_model)

    noise = NoiseModel()

    # Add depolarizing error to all single qubit gates and to all two qubit gates
    noise.add_all_qubit_quantum_error(depolarizing_error(noise_params["depol_one_qb_error"], 1), ['rx', 'ry', 'rz'])
    noise.add_all_qubit_quantum_error(depolarizing_error(noise_params["depol_two_qb_error"], 2), ['cx'])

    # Add amplitude damping error to all single qubit gates and to all two qubit gates
    noise.add_all_qubit_quantum_error(depolarizing_error(noise_params["amp_damp_one_qb_error"], 1), ['rx', 'ry', 'rz'])
    noise.add_all_qubit_quantum_error(depolarizing_error(noise_params["amp_damp_two_qb_error"], 2), ['cx'])

    # Add reset noise to all single qubit resets
    noise.add_all_qubit_quantum_error(reset_error(noise_params["reset_to_zero_error"], noise_params["reset_to_one_error"]), ["reset"])

    # Add readout error
    error_meas = ReadoutError([[1 - noise_params["p1given0_error"], noise_params["p1given0_error"]], [noise_params["p0given1_error"], 1 - noise_params["p0given1_error"]]])
    noise.add_all_qubit_readout_error(error_meas)

    # assign a quantum volume (measured using the values below)
    noise.QV = noise_params["QV"]

    return noise

def get_noise_parameters(noise_model=None):
    
    # import noise model
    filepath = '_common/custom/' + noise_model
...
def set_execution_target(backend_id='qasm_simulator', provider_module_name=None,
                         provider_name=None, provider_backend=None, hub=None,
                         group=None, project=None, noise_model=None,
    ...
    :param hub: hub identifier, currently "ibm-q" for IBM Quantum
...
def execute_circuit(circuit):
    ...
    try:
        ...
        # get noise model from options; used only in simulator for now
        if backend_exec_options_copy is not None and "noise_model" in backend_exec_options_copy:
            this_noise = backend_exec_options_copy["noise_model"]
            # print(f"... using custom noise model: {this_noise}")

        # extract execution options if set
        if backend_exec_options_copy is None: backend_exec_options_copy = {}
        ...
        # normal execution processing is performed here
            ...
            if this_noise is not None and not use_sessions and backend_name.endswith("qasm_simulator"):
                ...
                if transformer:
                    ...
                    simulation_circuits, shots = invoke_transformer(transformer, trans_qc,
            ...
            # Initiate execution for all other backends and noiseless simulator
                ...
                if transpile_attempt_count:
                    trans_qc = transpile_multiple_times(circuit["qc"], circuit["params"], backend,
                                                        transpile_attempt_count, optimization_level=None,
                ...
                # apply transformer pass if provided
                if transformer:
                    trans_qc, shots = invoke_transformer(transformer, trans_qc,
                                                         backend=backend, shots=shots)
